...
/*!
   ...
   \sa isValid(), inverted()
*/
GraphDoubleInterval GraphDoubleInterval::normalized() const
{
...
/*!
   ...
   \sa normalized()
*/
GraphDoubleInterval GraphDoubleInterval::inverted() const
{
    int borderFlags = 0;
    ...
    return GraphDoubleInterval(d_maxValue, d_minValue, borderFlags);
}
...
*/
bool GraphDoubleInterval::contains(double value) const
{
    if ( !isValid() )
        return false;
...
//! Unite 2 intervals
GraphDoubleInterval GraphDoubleInterval::unite(
    const GraphDoubleInterval &other) const
{
    /*
...
GraphDoubleInterval GraphDoubleInterval::intersect(
    const GraphDoubleInterval &other) const
{
    if ( !other.isValid() || !isValid() )
        return GraphDoubleInterval();

    GraphDoubleInterval i1 = *this;
    GraphDoubleInterval i2 = other;
    ...
    GraphDoubleInterval intersected;
    int flags = 0;
...
GraphDoubleInterval& GraphDoubleInterval::operator|=(
...
*/
bool GraphDoubleInterval::intersects(const GraphDoubleInterval &other) const
{
...
*/
GraphDoubleInterval GraphDoubleInterval::symmetrize(double value) const
{
    if ( !isValid() )
        return *this;

    const double delta =
        graphMax(graphAbs(value - d_maxValue), graphAbs(value - d_minValue));

    return GraphDoubleInterval(value - delta, value + delta);
}
...
*/
GraphDoubleInterval GraphDoubleInterval::limited(
    double lowerBound, double upperBound) const
{
    if ( !isValid() || lowerBound > upperBound )
        return GraphDoubleInterval();

    double minValue = graphMax(d_minValue, lowerBound);
    minValue = graphMin(minValue, upperBound);

    double maxValue = graphMax(d_maxValue, lowerBound);
    maxValue = graphMin(maxValue, upperBound);

    return GraphDoubleInterval(minValue, maxValue, d_borderFlags);
}
...
/*!
   ...
   \sa isValid()
*/
GraphDoubleInterval GraphDoubleInterval::extend(double value) const
{
    if ( !isValid() )
        return *this;

    return GraphDoubleInterval( graphMin(value, d_minValue),
        graphMax(value, d_maxValue), d_borderFlags );
}

GraphDoubleInterval& GraphDoubleInterval::operator|=(double value)
{
    *this = *this | value;
    return *this;
}
